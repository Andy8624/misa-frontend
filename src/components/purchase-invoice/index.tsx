...
export const PurchaseInvoice: React.FC = () => {
  ...
  const columns = [
    {
      title: <p className="whitespace-nowrap">Trạng thái kiểm tra</p>,
    ...
    {
      title: <p className="whitespace-nowrap">Tổng tiền thanh toán</p>,
    ...
    // },
    {
      ...
      render: (content: any) => (
        <div className="flex items-center gap-2">
          <Link
            href={`/popup/pu-voucher?invoice=` + content}
          ...
          <Popover
            ...
            content={
              <div className="flex flex-col">
                <Link
                  href={`/popup/pu-service?invoice=` + content}
                  className="p-1.5 font-medium"
                >
                  <p className="text-gray-800">Lập chứng từ mua dịch vụ</p>
                </Link>
                <Link
                  href={`/popup/cash-payment?invoice=` + content}
                  className="p-1.5 font-medium"
                >
                  <p className="text-gray-800">Lập phiếu chi tiền mặt</p>
                </Link>
                <Link
                  href={`/popup/ba-withdraw-payment?invoice=` + content}
                  className="p-1.5 font-medium"
                >
                  <p className="text-gray-800">Lập ủy nhiệm chi</p>
                </Link>
                <div
  ...
  const getListInvoice = async () => {
    try {
      const response = await axios.get(
        ...
        {
          params: {
            filter: { type: { _eq: "purchase_invoice" } },
          },
          headers: {
            Authorization: "Bearer " + process.env.NEXT_PUBLIC_API_ACCOUNTING_TOKEN,
          },
  ...
  return (
    <div>
      ...
      <Drawer
        title={
          <p className="text-xl font-semibold">Chi tiết hóa đơn đầu vào</p>
        }
        placement={"bottom"}
        closable={false}
