...
import { InventoryInQuantityValueRead } from "@/components/inventory-in-quantity-value-read";

export default function ShowInventoryIn() {
  const { id } = useParams();
  ...
  const getListInventoryIn = async () => {
    try {
      const response = await axios.get(
        process.env.NEXT_PUBLIC_API_ACCOUNTING_URL + "/items/inventory_in/" + id,
        {
          headers: {
      ...
      if (data) {
        const postDate = data?.posted_date ? new Date(data?.posted_date) : undefined;
        const voucherDate = data?.voucher_date ? new Date(data?.voucher_date) : undefined;
        setInventoryInVoucherType(data?.inventory_in_voucher_type || '');
        ...
        setWithOriginalVoucher(data?.with_original_voucher === 0 ? 0 : data?.with_original_voucher || undefined);
        setPostedDate(postDate ? `${postDate.getFullYear()}-${(postDate.getMonth() + 1) < 10 ? '0' + (postDate.getMonth() + 1) : (postDate.getMonth() + 1)}-${postDate.getDate() < 10 ? '0' + postDate.getDate() : postDate.getDate()}` : undefined);
        setVoucherDate(voucherDate ? `${voucherDate.getFullYear()}-${(voucherDate.getMonth() + 1) < 10 ? '0' + (voucherDate.getMonth() + 1) : (voucherDate.getMonth() + 1)}-${voucherDate.getDate() < 10 ? '0' + voucherDate.getDate() : voucherDate.getDate()}` : undefined);
        setStoreperson(data?.storeperson || undefined);
        setCustomer(data?.customer || undefined);
        setCostOfInventoryReturned(data?.cost_of_inventory_returned || undefined);
      }
    } catch (error) { }
  };

  const getListInventoryInItems = async () => {
    try {
      const response = await axios.get(
        process.env.NEXT_PUBLIC_API_ACCOUNTING_URL + "/items/inventory_in_item",
        {
          params: {
            filter: { inventory_in: { _eq: id } },
          },
          headers: {
            Authorization: "Bearer " + process.env.NEXT_PUBLIC_API_ACCOUNTING_TOKEN,
          },
