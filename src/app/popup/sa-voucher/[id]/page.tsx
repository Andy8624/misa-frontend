...
import { SalesVoucherDebtVoucherRead } from "@/components/sales-voucher-debt-voucher-read";
import { SalesVoucherCashReceiptVoucherRead } from "@/components/sales-voucher-cash-receipt-voucher-read";
import { SalesVoucherCashReceiptRead } from "@/components/sales-voucher-cash-receipt-read";
import { SalesVoucherInventoryOutVoucherRead } from "@/components/sales-voucher-inventory-out-voucher-read";
import { SalesVoucherInvoiceRead } from "@/components/sales-voucher-invoice-read";
import { SalesVoucherItemListRead } from "@/components/sales-voucher-item-list-read";
import { OPTION_SA_VOUCHER_PAYMENT_METHOD, OPTION_SA_VOUCHER_PAYMENT_STATUS, OPTION_SA_VOUCHER_SALES_TYPE } from "@/constants/constants";

export default function ShowSalesVoucher() {
  const { id } = useParams();
  ...
  const [salesVoucher, setSalesVoucher] = useState<any>();
  ...
  const getSalesVoucher = async () => {
    try {
      const response = await axios.get(
        process.env.NEXT_PUBLIC_API_ACCOUNTING_URL + "/items/sales_voucher/" + id,
        {
          headers: {
  ...
  const handleSelectSalesType = (value: string) => {
    ...
    if (
      ...
      value === OPTION_SA_VOUCHER_SALES_TYPE.ENTRUSTED_EXPORT_SERVICE_SALES.value
    ) {
  ...
  const handleSave = async () => {
    ...
    if (
      saleType === OPTION_SA_VOUCHER_SALES_TYPE.ENTRUSTED_EXPORT_SERVICE_SALES.value ||
          ...
          <div className="h-9 w-[340px] border rounded overflow-hidden ml-8 border-neutral-300">
            <Select
              ...
              options={Object.keys(OPTION_SA_VOUCHER_SALES_TYPE).map(
                (key, idx: number) => ({
                  label: `${idx + 1}. ${OPTION_SA_VOUCHER_SALES_TYPE[key as keyof typeof OPTION_SA_VOUCHER_SALES_TYPE]
                    .translate.vi
                    }`,
                  value:
                    OPTION_SA_VOUCHER_SALES_TYPE[key as keyof typeof OPTION_SA_VOUCHER_SALES_TYPE]
                      .value,
                })
              )}
      ...
      <div className="flex-1 relative">
        <div className="w-full h-full absolute top-0 left-0 p-4 overflow-y-auto">
          <div className="flex items-center gap-4">
            <div
              className={`flex items-center gap-4 ${(saleType === OPTION_SA_VOUCHER_SALES_TYPE.EXPORTED_GOODS_SALES.value ||
                saleType ===
            ...
            >
              <div className="flex items-center">
                <input
                  ...
                  checked={
                    paymentStatus === OPTION_SA_VOUCHER_PAYMENT_STATUS.ON_CREDIT.value
                  }
                  onChange={(e) => {
              ...
              <div className="flex items-center">
                <input
                  checked={paymentStatus === OPTION_SA_VOUCHER_PAYMENT_STATUS.PAID.value}
                  type="radio"
              ...
              <div
                className={`h-10 w-44 border rounded-md overflow-hidden ${paymentStatus !== OPTION_SA_VOUCHER_PAYMENT_STATUS.PAID.value &&
                  "pointer-events-none"
                  }`}
              >
                <Select
                  ...
                  options={Object.keys(OPTION_SA_VOUCHER_PAYMENT_METHOD).map((key) => ({
                    label:
            ...
            <Tabs
              ...
              items={[
                ...
                },
                {
                  ...
                  active:
                    paymentStatus === OPTION_SA_VOUCHER_PAYMENT_STATUS.PAID.value &&
                      ...
                      : false,
                  children: (
                    <SalesVoucherCashReceiptVoucherRead
                      listEmployee={listEmployee}
                      listCustomer={listPartner}
                ...
                },
                {
                  ...
                  children: (
                    <SalesVoucherInventoryOutVoucherRead
                      listEmployee={listEmployee}
                      listCustomer={listPartner}
